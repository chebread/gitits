...
const Chart = ({ contributions, element }) => {
  const promise = loadScript('https://cdn.jsdelivr.net/npm/chart.js');
  promise.then(() => {
    const { Chart } = window;
    const dates = [...Object.keys(contributions)];
    const datas = [...Object.values(contributions)];
    const isOneData = Object.keys(contributions).length === 1 ? true : false;
    const data = {
      labels: isOneData ? [...dates, ...dates] : [...dates], // label만 표시하는 것만 월만 제공하게 합니다!
      datasets: [
        {
          label: 'Contribution',
          data: isOneData ? [...datas, ...datas] : [...datas],
          borderColor: 'rgb(65, 105, 225)',
          backgroundColor: 'rgba(0, 0, 0, 0)', // 무색
          pointBorderColor: 'rgba(0, 0, 0, 0)',
          pointStyle: 'circle',
          pointRadius: 25,
          pointHoverRadius: 30, // 이것은 배경은 없지만 hover된 상태에서는 크기가 남아있기 때문에 그 point를 벗어나더라도 이 크기의 범위 내에서는 유지가 되므로 재밌는 인터렉티브한 animation을 구현할 수 있다
        },
      ],
      grid: {
